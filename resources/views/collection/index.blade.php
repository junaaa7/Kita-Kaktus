...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-6">
        <form method="GET" action="{{ route('collection.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <!-- Filter Kategori -->
            <div>
            ...
            <!-- Filter Harga Min -->
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Harga Min (Rp)</label>
            ...
            <!-- Filter Harga Max -->
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Harga Max (Rp)</label>
            ...
            <div>
                ...
                <select name="sort" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            ...
            <div class="flex items-end space-x-2">
                ...
                <a href="{{ route('collection.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition">
    ...
    <div class="mb-4 flex justify-between items-center">
        <p class="text-gray-600 dark:text-gray-400">Menampilkan {{ $products->firstItem() ?? 0 }} - {{ $products->lastItem() ?? 0 }} dari {{ $products->total() }} produk</p>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-xl transition transform hover:-translate-y-1 duration-300">
            ...
            <div class="p-4">
                ...
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-2 line-clamp-1">{{ $product->name }}</h3>
                <p class="text-gray-600 dark:text-gray-400 text-sm mb-2 line-clamp-2">{{ Str::limit($product->description, 80) }}</p>
                <div class="flex justify-between items-center mb-3">
                    <span class="text-green-600 dark:text-green-400 font-bold text-xl">Rp {{ number_format($product->price, 0, ',', '.') }}</span>
                ...
                @auth
                    @if($product->stock > 0)
                        <form action="{{ route('cart.add', $product) }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg transition font-semibold">
                    ...
                    @endif
                @else
                    <a href="{{ route('login') }}" class="block w-full bg-green-600 hover:bg-green-700 text-white text-center py-2 rounded-lg transition font-semibold">
                        <i class="fas fa-sign-in-alt mr-2"></i> Login untuk Beli
                    </a>
                @endauth
                
                <a href="{{ route('product.detail', $product) }}" class="block text-center mt-2 text-green-600 dark:text-green-400 hover:text-green-700 text-sm">
                    Lihat Detail →
                </a>
            </div>
        </div>
