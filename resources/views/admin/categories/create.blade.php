...
<div class="max-w-2xl mx-auto px-3 sm:px-0">
    <h1 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white mb-4 sm:mb-6">Tambah Kategori Baru</h1>
    
    <form action="{{ route('admin.categories.store') }}" method="POST" class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 sm:p-6">
        @csrf
        ...
        <div class="mb-6">
            ...
            <textarea name="description" rows="4" class="w-full px-3 py-2 text-sm sm:text-base border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-green-500">{{ old('description') }}</textarea>
        </div>
